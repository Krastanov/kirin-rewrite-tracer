...
- **Open when:** Designing, implementing, or reviewing event-row clicks, contiguous
  range selection, parent dominance, descendant hiding, subtree collapse, or selection
  restoration.
...
- **Related specification IDs:** SYS-020, SYS-025
- **Review when:** Selection gestures, range-anchor behavior, dominance, collapse
  eligibility, restoration, or selection accessibility changes.
...
1. **Collapse only ever hides.** A row is displayed exactly when no selected ancestor
   and no collapsed ancestor hides it. Because the two rules compose as a union, no
   collapse state can reveal a row that parent dominance hides, and SYS-020's "no
   disclosure state reveals a hidden descendant" clause still holds.
